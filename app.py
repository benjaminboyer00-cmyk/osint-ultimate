...
import os, json, re, socket, threading, queue, time, random
...
def scan_ip(ip):
    ...
    try:
        ...
        if geo.get('status') == 'success':
            return {'Pays':geo['country'],'Ville':geo['city'],'FAI':geo['isp'],'Lat':geo['lat'],'Lon':geo['lon']}
    except: pass
...
# Ajoute les autres modules (instagram, twitter, etc.) sur le même modèle...
def scan_instagram(username):
    try:
        r = requests.get(f'https://www.instagram.com/{username}/?__a=1', headers={'User-Agent':'Mozilla/5.0'}, timeout=10)
