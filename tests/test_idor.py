...
@pytest.fixture
def two_users(idor_app):
  owner = User(username='owner_idor', email='o@t.com')
  owner.set_password('OwnerPass2024!xyz')
  intruder = User(username='intruder_idor', email='i@t.com')
  intruder.set_password('IntruderPass2024!xyz')
  db.session.add_all([owner, intruder])
  db.session.commit()
  dossier = Entity(user_id=owner.id, value='cible@test.com', entity_type='email')
  db.session.add(dossier)
  db.session.commit()
  return owner, intruder, dossier


def test_get_dossier_context_blocks_intruder(two_users):
    from services.dossier_access import get_dossier_context
    owner, intruder, dossier = two_users
...
def test_dossier_route_403_for_intruder(idor_app, two_users):
    owner, intruder, dossier = two_users
    client = idor_app.test_client()
    with client.session_transaction() as sess:
        pass
    client.post('/login', data={'username': 'intruder_idor', 'password': 'IntruderPass2024!xyz'})
    r = client.get(f'/expert/dossier/{dossier.id}')
    assert r.status_code in (403, 404)


def test_scan_result_idor_on_private_scan(idor_app, two_users):
    owner, intruder, dossier = two_users
    scan = Scan(
        user_id=owner.id,
        module='email',
...
def test_scan_result_allowed_via_shared_dossier(idor_app, two_users):
    owner, intruder, dossier = two_users
    scan = Scan(
        user_id=owner.id,
        module='email',
